...
export default new Command(
  ...
  (message, args, client) => {
    if (!args.length) {
      message.channel.createMessage({
        embed: {
          ...
          description: `${client.commands
            .map((c) => `\`${client.prefix}${c.names[0]}\``)
            .join("\n")}\n\nWant more info about a command? Run the \`${
            client.prefix
      ...
      if (command.expectedArguments)
        helpMessage += `**Command Usage**: \`${client.prefix}${command.names[0]} ${command.expectedArguments}\`\n`;

      if (command.names.length > 1)
        helpMessage += `**Aliases**: \`${command.names
          .slice(1)
          .map((c) => `${client.prefix}${c}`)
          .join(", ")}\``;
