...
import { InteractionPacketD, Interaction } from "../utils/Interaction";
...
export default new Event("rawWS", (packet, _id, client) => {
  ...
  if (interaction.type === 2) {
    client.emit(
      ...
      new SlashInteraction(packet.d as InteractionPacketD),
      client
    );
  } else {
    client.emit(
      "componentInteractionCreate",
      new ComponentInteraction(packet.d as InteractionPacketD),
      client
    );
  }
});
